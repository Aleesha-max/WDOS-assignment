if(localStorage.getItem('role')){
    ...
    document.querySelectorAll('*[x-text]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-text').split("$store.animals.");
    ...
    document.querySelectorAll('*[x-html]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-html').split("$store.animals.");
    ...
    document.updateObjectData = (path, value) => {
        ...
        let obj = JSON.parse(localStorage.getItem('animals'));
        let currentObj = obj;
        console.log(obj);
        ...
        localStorage.setItem('animals', JSON.stringify({ ...obj, currentObj }))
        Alpine.store('animals', { ...obj, currentObj })
    }
