if(localStorage.getItem('role')){
    ...
    document.querySelectorAll('*[x-text]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-text').split("$store.conservation.");
    ...
    document.querySelectorAll('*[x-html]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-html').split("$store.conservation.");
    ...
    document.updateObjectData = (path, value) => {
        ...
        let obj = JSON.parse(localStorage.getItem('conservation'));
        let currentObj = obj;
        console.log(obj);
        ...
        localStorage.setItem('conservation', JSON.stringify({ ...obj, currentObj }))
        Alpine.store('conservation', { ...obj, currentObj })
    }
