if(localStorage.getItem('role')){
    ...
    document.querySelectorAll('*[x-text]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-text').split("$store.introduction.");
    ...
    document.querySelectorAll('*[x-html]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-html').split("$store.introduction.");
    ...
    document.updateObjectData = (path, value) => {
        ...
        let obj = JSON.parse(localStorage.getItem('introduction'));
        let currentObj = obj;
        console.log(obj);
        ...
        localStorage.setItem('introduction', JSON.stringify({ ...obj, currentObj }))
        Alpine.store('introduction', { ...obj, currentObj })
    }
