if(localStorage.getItem('role')){
    ...
    document.querySelectorAll('*[x-text]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-text').split("$store.wilpattu.");
    ...
    document.querySelectorAll('*[x-html]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-html').split("$store.wilpattu.");
    ...
    document.updateObjectData = (path, value) => {
        ...
        let obj = JSON.parse(localStorage.getItem('wilpattu'));
        let currentObj = obj;
        console.log(obj);
        ...
        localStorage.setItem('wilpattu', JSON.stringify({ ...obj, currentObj }))
        Alpine.store('wilpattu', { ...obj, currentObj })
    }
