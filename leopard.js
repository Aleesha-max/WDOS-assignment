if(localStorage.getItem('role')){
    ...
    document.querySelectorAll('*[x-text]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-text').split("$store.leopard.");
    ...
    document.querySelectorAll('*[x-html]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-html').split("$store.leopard.");
    ...
    document.updateObjectData = (path, value) => {
        ...
        let obj = JSON.parse(localStorage.getItem('leopard'));
        let currentObj = obj;
        console.log(obj);
        ...
        localStorage.setItem('leopard', JSON.stringify({ ...obj, currentObj }))
        Alpine.store('leopard', { ...obj, currentObj })
    }
