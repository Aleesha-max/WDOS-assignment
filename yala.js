if(localStorage.getItem('role')){
    ...
    document.querySelectorAll('*[x-text]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-text').split("$store.yala.");
    ...
    document.querySelectorAll('*[x-html]').forEach((item) => {
        ...
        editOverlay.addEventListener("click",()=>{
            const path = item.getAttribute('x-html').split("$store.yala.");
    ...
    document.updateObjectData = (path, value) => {
        ...
        let obj = JSON.parse(localStorage.getItem('yala'));
        let currentObj = obj;
        console.log(obj);
        ...
        localStorage.setItem('yala', JSON.stringify({ ...obj, currentObj }))
        Alpine.store('yala', { ...obj, currentObj })
    }
